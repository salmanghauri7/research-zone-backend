...
export default class chatController{
    static async handleMessagesWithPagination(req, res){
        try {
            ...
            const {messages, nextCursor} = await chatServices.getMessagesbyPagination(workspaceId, limit, cursor);

            return apiResponse.success(
                res,
                successMessages.CHAT?.MESSAGES_FETCHED || "Messages fetched successfully",
                200,
                {
                    messages,
                    cursor: nextCursor,
            ...
            return apiResponse.error(
                res,
                err.message || errorMessages.CHAT?.FETCH_MESSAGES_FAILED || "Failed to fetch messages",
                err.statusCode || 500
            );
        }
    }
